...
const props = defineProps<{ draft: CaptureProfileInput | null; currentClientVersion: string }>()
...
const clientVersionMin = ref('')
const clientVersionMax = ref('')
...
watch(() => props.currentClientVersion, (version) => {
  if (version && version !== 'unknown' && clientVersion.value === 'unknown') clientVersion.value = version
})

function populateEditor(profile: CaptureProfile): void {
  editingProfileId.value = profile.id
  profileName.value = profile.name
  theme.value = profile.theme
  clientName.value = profile.client.name
  clientVersion.value = profile.client.version
  clientVersionMin.value = profile.compatibility.clientVersion.min ?? ''
  clientVersionMax.value = profile.compatibility.clientVersion.max ?? ''
...
function beginNewProfile(): void {
  ...
  const sourceName = props.draft?.source.name
  profileName.value = sourceName ? `${sourceName} Profile` : '新 Profile'
  matchValue.value = sourceName ?? ''
  clientVersion.value = props.currentClientVersion || 'unknown'
  clientVersionMin.value = ''
  clientVersionMax.value = ''
  message.value = '已切换到新建模式'
}
...
async function saveProfile(): Promise<void> {
  ...
  isBusy.value = true
  message.value = null
  const editing = editingProfileId.value ? profiles.value.find((profile) => profile.id === editingProfileId.value) : undefined
  const result = await window.chessMonitor.profiles.save({
    ...props.draft,
    ...(editingProfileId.value ? { id: editingProfileId.value } : {}),
    thresholds: editing?.thresholds ?? props.draft.thresholds,
    stableFrameRequirement: editing?.stableFrameRequirement ?? props.draft.stableFrameRequirement,
    animationWaitMs: editing?.animationWaitMs ?? props.draft.animationWaitMs,
    name: profileName.value,
    theme: theme.value,
    client: { name: clientName.value, version: clientVersion.value },
    compatibility: {
      dpi: { min: dpiMin.value, max: dpiMax.value },
      frameScale: editing?.compatibility.frameScale ?? { min: 0.5, max: 2 },
      clientVersion: {
        min: clientVersionMin.value.trim() || null,
        max: clientVersionMax.value.trim() || null,
      },
    },
    priority: priority.value,
    isEnabled: editing?.isEnabled ?? true,
    matchRules: [
      { mode: matchMode.value, value: matchValue.value || props.draft.source.name },
      ...(editing?.matchRules.slice(1) ?? []),
    ],
    model: editing?.model ?? { strategy: 'shared', modelVersion: null },
  })
  if (!result.ok) {
    isBusy.value = false
  ...
  populateEditor(activated.value)
...
async function deleteProfile(profile: CaptureProfile): Promise<void> {
  ...
  if (result.ok && result.value.deleted && editingProfileId.value === profile.id) beginNewProfile()
  await loadProfiles()
}
...
async function toggleProfile(profile: CaptureProfile): Promise<void> {
  ...
  if (result.ok && !result.value.isEnabled && activeProfileId.value === profile.id) activeProfileId.value = null
  await loadProfiles()
}
...
async function rollbackProfile(profile: CaptureProfile): Promise<void> {
  ...
  await loadProfiles()
  if (result.ok && editingProfileId.value === profile.id) populateEditor(result.value)
  if (result.ok && activeProfileId.value === profile.id) emit('apply', result.value)
}
  ...
  <section class="profile-panel" aria-labelledby="profile-title">
    <div class="section-heading">
      ...
      <div class="profile-toolbar">
        <button type="button" class="secondary-action" :disabled="isBusy" @click="beginNewProfile">新建</button>
    ...
    <div class="profile-editor">
      ...
      <label for="profile-client-version-min">兼容版本范围</label>
      <div class="inline-fields"><input id="profile-client-version-min" v-model.trim="clientVersionMin" maxlength="64" placeholder="最低（可空）" /><span>–</span><input v-model.trim="clientVersionMax" maxlength="64" placeholder="最高（可空）" aria-label="最高兼容客户端版本" /></div>
      ...
      <button type="button" class="primary-action" :disabled="!draft || isBusy" @click="saveProfile">
        {{ isBusy ? '正在处理…' : editingProfileId ? '保存为下一版本' : '保存新 Profile' }}
      </button>
      <small>{{ editingProfileId ? `正在编辑 ${editingProfileId}` : '新建模式' }}</small>
    </div>
